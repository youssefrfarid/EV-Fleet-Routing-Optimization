...
class VehicleSolution:
    ...
    # Optional: speed in km/h for each edge (continuous, default 80.0)
    speed_levels: Dict[Tuple[str, str], float] = None

    def get_speed_level(self, edge: Tuple[str, str]) -> float:
        """
        Get the speed chosen for a specific edge.

        Args:
            edge: Tuple of (source_node, destination_node)

        Returns:
            Speed in km/h, defaults to 80.0 if not specified
        """
        if self.speed_levels is None:
            return 80.0  # Default to reference speed
        return self.speed_levels.get(edge, 80.0)
...
class FleetSolution:
    ...
    def is_feasible(self, verbose: bool = False, return_reason: bool = False):
        ...
        Args:
            ...
            return_reason: If True, return a tuple (feasible, code, message)
                where code maps to the violated constraint.

        Returns:
            - When return_reason is False (default): bool indicating feasibility.
            - When return_reason is True: (bool, int, str) tuple containing
              the feasibility flag, diagnostic code, and human-readable message.
        """

        self._last_feasibility = (None, None, None)

        def make_response(feasible: bool, code: int, message: str):
            self._last_feasibility = (feasible, code, message)
            if return_reason:
                return feasible, code, message
            return feasible
        ...
        for i, vs in enumerate(self.vehicle_solutions):
            ...
            if not self._check_speed_levels(vs, verbose, i):
                return make_response(False, 7, "Invalid speed level selection")
    ...
    def _check_speed_levels(self, vs: VehicleSolution, verbose: bool, vehicle_idx: int) -> bool:
        """Check if all speeds are within valid bounds for all edges in route."""
        if vs.speed_levels is None:
            # No speeds specified, defaults to reference speed - OK
            return True
        ...
        for i in range(len(vs.route) - 1):
            ...
            if edge in vs.speed_levels:
                ...
                # Get edge-specific speed bounds
                min_speed, max_speed = self.params.get_edge_speed_bounds(edge)
                
                # Check bounds [edge_min, edge_max]
                if speed < min_speed or speed > max_speed:
                    if verbose:
                        print(
                            f"❌ Vehicle {vehicle_idx}: Speed {speed:.1f} km/h out of bounds for edge {edge}"
                        )
                        print(
                            f"   Valid range for this edge: [{min_speed:.1f}, {max_speed:.1f}] km/h"
                        )
                    return False
